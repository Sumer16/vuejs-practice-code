      ...
      <form @submit.prevent="onSubmit">
        ...
        <div class="input" :class="{invalid: $v.password.$error}">
          ...
          <input
                  ...
                  v-model="password">
          <p v-if="!$v.password.minLen">Please select a password greater than {{ $v.password.$params.minLen.min }} characters.</p>
        </div>
...
import axios from 'axios';
  ...
  export default {
    ...
    validations: {
      email: {
        required, //required: required 1st one can be of any name & 2nd one is the dependency
        email,
        unique: val => {
          if(val === '') return true;
